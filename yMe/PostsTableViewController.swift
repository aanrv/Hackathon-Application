...
class PostsTableViewController: PFQueryTableViewController {
    ...
    override init(style: UITableViewStyle, className: String!)
    {
        ...
        let postButton:UIButton = UIButton(type: UIButtonType.System)
        ...
        let btnImage = UIImage(named: "plus.png")
        postButton.setImage(btnImage, forState: UIControlState.Normal)
        ...
        let trendingButton:UIButton = UIButton(type: UIButtonType.System)
        trendingButton.frame = CGRectMake(self.view.frame.size.width * 0.2, self.view.frame.size.height / 2, toolbar.frame.size.height / 2, toolbar.frame.size.height / 2)
        trendingButton.addTarget(self, action: "loadTrending", forControlEvents: UIControlEvents.TouchUpInside)                                 //CHANGE
        trendingButton.layer.zPosition = 100
        trendingButton.setImage(UIImage(named: "trendingIcon"), forState: UIControlState.Normal)
        
        let newestButton:UIButton = UIButton(type: .System)
        newestButton.frame = CGRectMake(self.view.frame.size.width * 0.8 , self.view.frame.size.height / 2, toolbar.frame.size.height / 2, toolbar.frame.size.height / 2)                               //CHANGE
        newestButton.addTarget(self, action: "loadNewest", forControlEvents: UIControlEvents.TouchUpInside)
        newestButton.layer.zPosition = 100
        newestButton.setImage(UIImage(named: "recentIcon"), forState: UIControlState.Normal)
    
        toolbar.setItems([flexibleSpace, UIBarButtonItem(customView: trendingButton), flexibleSpace, UIBarButtonItem(customView: postButton), flexibleSpace, UIBarButtonItem(customView: newestButton), flexibleSpace], animated: true)
    ...
    func showPostView() {
        ...
        let v = PostStoryViewController(numPosts: rows)
        v.view.backgroundColor = UIColor.lightGrayColor()
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if let tmpObj = referringObject {
            ...
            test.navigationItem.title = "Article"
    ...
    override func queryForTable() -> PFQuery {
        ...
        let sortId = (onTrendingView) ? "likes" : "uid"
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath, object: PFObject?) -> PFTableViewCell? {
        ...
        if let pfObject = object {
            ...
            cell?.detailTextLabel?.text = "Points " + String(likes) + " • Comments: " + String(numComments) + " • Category: " + intToCat(pfObject.valueForKey("category") as! Int)
    ...
    func loadTrending(){
        self.onTrendingView = true
        self.loadObjects()
    }
    
    
    func loadNewest(){
        self.onTrendingView = false
        self.loadObjects()
    }
